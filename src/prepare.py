...
with mlflow.start_run() as run:
    ...
    for image in list_img_validation:
        ...
        shutil.copy(
            f"{path_dataset_images}/{image}", f"{path_validation_images}/{image}"
        )
    ...
    image_brightness_handler = cv_utils.ImageBrightnessHandler(
        cv_utils.NoneImageStrategy()
    )
    ...
    with open(
        str(params["annotations_directory"] + "/classes.txt"), "r"
    ) as classes_file:
    ...
    mlflow.log_param(
        "adjust_images_brightness_strategy", adjust_images_brightness_strategy
    )
